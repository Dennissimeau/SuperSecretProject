...
final class SuperSecretProjectTests: XCTestCase {
    ...
    func testFetchItemsFails() async {
        ...
        XCTAssertEqual(viewModel.locations.count, 0)
        XCTAssertEqual(viewModel.loadState, .error(errorMessage: NetworkError.decodingFailed.errorDescription))
        XCTAssertNotEqual(viewModel.loadState, .error(errorMessage: NetworkError.invalidURL.errorDescription))
    }
